# Title: Nov detection with all 9 and then 9 + size
...
sub_taxonomic_matrix_list <- seasonal.matrix.maker(survey_data, ts_data)

# 3.  Make func dissims matrix for 10 measurements --------------------------------------------

# Subset rar
func9_dissims_matrix <- as.matrix(gowdis(trait_data[,4:12]), 
                                  dimnames = list(full_trait_data$Species))
dimnames(func9_dissims_matrix) <- list(trait_data$Species, trait_data$Species)
# Need to make a multitrait dissimilarity matrix between all species using gowdis
func10_dissims_matrix <- as.matrix(gowdis(trait_data[,3:12]), 
                                 dimnames = list(full_trait_data$Species))
dimnames(func10_dissims_matrix) <- list(trait_data$Species, trait_data$Species)
attributes(func10_dissims_matrix) # All traits should be numeric

# Change to numeric
class(func9_dissims_matrix) <- "numeric"
class(func10_dissims_matrix) <- "numeric"

# See first 5 rows and cols of diss matrix
func_dissims_matrix[1:5, 1:5]

# Save func dissims matrix as csv if needed later
write.csv(func9_dissims_matrix, "./inputs/data/func9_dissims_matrix.csv")
write.csv(func10_dissims_matrix, "./inputs/data/func10_dissims_matrix.csv")

# 3.  Functional novelty detection ----------------------------------------

# Apply framework using functional novelty detection function
func9_results_list <- func.novelty.detection(sub_taxonomic_matrix_list, 
                                            func9_dissims_matrix)
